...
from utils.model.net import make_norm, freeze_params
from cfgs.CIHP_cfg import cfg

__all__ = ["ResNet_C5_Dilation"]
...
class ResNet_C5_Dilation(ResNet):
    def __init__(self, norm='bn', stride=32):
        super(ResNet_C5_Dilation, self).__init__()
        if cfg.RESNET_USE_ALIGN:
            block = AlignedBottleneck
        else:
        ...
        self.stride_3x3 = cfg.RESNET_STRIDE_3X3
        self.avg_down = cfg.RESNET_AVG_DOWN

        layers = cfg.RESNET_LAYERS[:int(math.log(stride, 2)) - 1]
        self.layers = layers
        self.base_width = cfg.RESNET_WIDTH

        stage_with_context = cfg.RESNET_STAGE_WITH_CONTEXT
        self.ctx_ratio = cfg.RESNET_CTX_RATIO
        stage_with_conv = cfg.RESNET_STAGE_WITH_CONV
        c5_dilation = cfg.RESNET_C5_DILATION

        self.inplanes = 64
        self.use_3x3x3stem = cfg.RESNET_USE_3x3x3HEAD
    ...
    def _init_modules(self):
        assert cfg.RESNET_FREEZE_AT in [0, 2, 3, 4, 5]  # cfg.RESNET_FREEZE_AT: 2
        assert cfg.RESNET_FREEZE_AT <= len(self.layers) + 1
        if cfg.RESNET_FREEZE_AT > 0:
        ...
        for i in range(1, cfg.RESNET_FREEZE_AT):
    ...
    def train(self, mode=True):
        ...
        for i in range(cfg.RESNET_FREEZE_AT, len(self.layers) + 1):
            if i == 0:
                continue
